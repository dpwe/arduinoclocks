...
void MCP79412RTC::write(tmElements_t &tm)
{
    ...
    i2cWrite(tm.Wday | _BV(VBATEN));             // enable battery backup operation, resets powerfail flag
...
// Read all nine status/control registers and return as 9 raw bytes.
void MCP79412RTC::registersRead(byte *nine_bytes)
{
    i2cBeginTransmission(RTC_ADDR);
    i2cWrite((uint8_t)TIME_REG);
    i2cEndTransmission();
    i2cRequestFrom( RTC_ADDR, 9 );
    for (byte i=0; i<9; i++) nine_bytes[i] = i2cRead();
}
