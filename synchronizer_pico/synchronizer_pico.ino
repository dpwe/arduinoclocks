...
#define NUM_BUTTONS 3   // on D9, D6, D5 on feather OLED wing
int button_pins[NUM_BUTTONS] = {btn_a_pin, btn_b_pin, btn_c_pin};
...
//#define FEATHER_OLED
#ifdef FEATHER_OLED
Adafruit_SH1107 display = Adafruit_SH1107(64, 128, &Wire);  // feather wing 64x128
...
void loop() {
  ...
  if(gps_clock.synced_ && 
     ((gps_clock.base_unixtime_ - sys_secs) < GPS_TIMEOUT_SECS)) {
    sys_clock = &gps_clock;
  } else {
    sys_clock = &rtc_clock;
  }
