...
//
// This version is adapted from BurstClock but goes back to steady advance
// (i.e., stepping every 1/2 sec or so) so as to avoid the half-second hum 
// every minute.  The price of this is that the minute falls are now not exact
// (due to nonuniformity in the flip mechanism) but each hour should be same.
//
// The approach is to treat each *step* as a regular event, and to track these 
// in the battery-backed RAM to tolerate power loss.  So rather than 1440 (one-minute)
// states per day, we now have 2048 (steps per rev) * 1440/12 (revs of the output pinion
// per day) = 245,760 states per day.  That's 18 bits, 
//
