...
// Current steps within revolution of stepper
int _stepper_state = 0;
volatile long _steps_to_do = 0;
// Semaphore by which client requests more steps.
...
void next_step_callback(void)
{
  ...
  if (steps_to_do_semaphore) {
    _steps_to_do += steps_to_do_semaphore;
    steps_to_do_semaphore = 0;
  }
  if (_steps_to_do) {
    stepper_active = 1;
    timeout_restart(MS_BETWEEN_STEPS);
    _stepper_state = (_stepper_state + 1) % STEPS_PER_REVOLUTION;
...
#define MINUTES_PER_REVOLUTION 12
// Keep track of where we are within the 12 minute revolution.
int current_minute = 0;
// Keep track of where we are within the 2048/2052 step revolution.
...
void advance_one_minute(void) {
  ...
  unsigned int new_step = (((long)current_minute * STEPS_PER_REVOLUTION) / MINUTES_PER_REVOLUTION);
  // Command the new steps to happen.
  add_steps(new_step - current_step);
  // Update state.
  current_minute = current_minute % MINUTES_PER_REVOLUTION;
  current_step = new_step % STEPS_PER_REVOLUTION;
}
...
void set_backlight_brightness(byte val) {
  // This setup has no backlight control, so this is a stub.
}
...
void loop() {
  if (++loop_count == 5) {
    // Only check the RTC every 5 loops i.e. 50ms.
    RTC_update();
    loop_count = 0;
  }
  flip_update();
