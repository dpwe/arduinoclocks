...
// This version is tuned for the Chronotel which needs 36 teeth
// per minute (3.6 revs of the stock telechron motor with a 10 
// tooth pinion; 3 revs of our stepper + 12 tooth pinion).
//
// It also uses the Adafruit Motor Shield to drive a 200 step NEMA17
// stepper motor.
...
// GE 7-4305A Chronotel
//#define STEPS_PER_CYCLE 6144   // 2048 step-per-cycle stepper
//#define STEPS_PER_CYCLE 1539     // 513 step-per-cycle stepper.  
// Adafruit claims 32*16.032 = 513.024, which would means we fall behind half a revolution per week?
// TODO: To correct, we should add 3 steps every 125 revolutions.
#define STEPS_PER_CYCLE 600     // 200 step-per-cycle stepper.  
#define MINUTES_PER_CYCLE 1
...
// Accelerated nema17 driving
// per https://forums.adafruit.com/viewtopic.php?f=31&t=57041&p=292119
// including quickstep function added to Adafruit_Motorshield.{h,cpp}
...
void stepper_update(void)
{
  ...
  if (steps_to_do) {
    stepper_active = 1;
    int steps_this_time = min(steps_to_do, STEPS_PER_UPDATE);
    int micros_per_step = 350;
    for (int i = 0; i < steps_this_time; i++) {
      myMotor->quickstep(BACKWARD);
      delayMicroseconds(micros_per_step);
    }
    //myMotor->step(steps_this_time, BACKWARD, DOUBLE);
    steps_to_do -= steps_this_time;
  } else {
...
void add_steps(int steps) {
  ...
  if (!stepper_active) {
    // Calling stepper_update will accept the semaphore, take a step immediately, then restart timer.
    stepper_update();
  }
}
...
// Chronotel mechanism only has a 12 h cycle
#define MINS_PER_DAY 720
...
void flip_set_state_mins(int mins) {
  ...
  if (mins < 0)
    return;
  display_mins_shown = mins % MINS_PER_DAY;
  Serial.print("Warp ");
  flip_report_display(display_mins_shown);
...
{
  requested_time = new_mins % MINS_PER_DAY;
}
